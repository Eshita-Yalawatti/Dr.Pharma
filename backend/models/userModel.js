...
const userSchema = new mongoose.Schema({
  ...
  //  Cart with discount and payment status
  cart: [
    {
...
// ---------------- Methods (Optional) ----------------
userSchema.methods.addToCart = async function (drugId, quantity, price) {
  const idx = this.cart.findIndex(
    (i) => i.drugId.toString() === drugId.toString()
  );
  if (idx >= 0) this.cart[idx].quantity += quantity;
  else this.cart.push({ drugId, quantity, price, discount: 0, isPaid: false });
  return this.save();
};

userSchema.methods.removeFromCart = async function (drugId) {
...
userSchema.methods.addOrderHistory = async function (orderId) {
  this.orderHistory.push(orderId);
  await this.save();
};

userSchema.methods.applyDiscount = async function (drugId, discountAmount) {
  const item = this.cart.find((i) => i.drugId.toString() === drugId.toString());
  if (item) item.discount = discountAmount;
  return this.save();
};

userSchema.methods.markPaid = async function () {
  this.cart = this.cart.map((i) => ({ ...i, isPaid: true }));
  return this.save();
};
